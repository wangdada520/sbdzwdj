      ...
      <div class="form_Department_cont">
        ...
        <div class="form_Department_center">
          <div class="form_Department_search">
            ...
            <input v-model="search" type="text" placeholder="请输入单位名称" @blur="getList(Module)">
            ...
            <ul v-if="listShow" class="form_Department_list">
              <li v-for="(item,index) in move" :key="index" :class="item.isok?'form_Department_list_on':''" @click="clickDepartment(item,index)">
                <div class="form_Department_list_txt">
                  <p><span>部门名称：</span><span>{{ item.deptname }}</span></p>
                  <p><span>部门描述：</span><span>{{ item.remark }}</span></p>
                </div>
        ...
        <div class="form_Department_right">
          <div class="form_Department_search">已选单位</div>
          <div class="form_Department_content">
...
import { list, getSysDepartment } from '@/api/SysDepartment'
export default {
  name: 'Index',
  components: {
    DepartmentTree
  ...
  methods: {
    ...
    handleNo() {
      ...
      this.move.map((item, index) => {
        item.isok = this.onSelectionString.join().indexOf(`"departmentid":"${item.departmentid}`) !== -1
        thismove.push(item)
    ...
    getPerArchives(id, isend) {
      ...
      getSysDepartment(id, params).then(res => {
    ...
    getList(val) {
      ...
      this.loading = true
      this.Module = val
      const search = { deptname: this.search, parentdeptid: val.id || 'ROOT' }
      const params = {
        pageNum: this.pages.pageNum,
        pageSize: this.pages.pageSize,
        search: String(this.search) ? search : { parentdeptid: val.id || 'ROOT' }
      }
      list(params).then((res) => {
        ...
        this.move = [{
          ...
          deptname: val.name,
          leaf: val.leaf,
          levelpath: val.levelpath,
          remark: '',
          isok: this.SelectionString.join().indexOf(`"departmentid":"${val.id}`) !== -1
        }]
        res.data.map((item, index) => {
          item.isok = this.SelectionString.join().indexOf(`"departmentid":"${item.departmentid}`) !== -1
          this.move.push(item)
        })
      }).catch(() => {
        this.loading = false
      ...
      if (this.issingle) {
        if (move.isok) {
          if (this.Selection.length > 0) {
            this.move.map((item01, index01) => {
              if (item01.departmentid === this.Selection[0].departmentid) {
                const move01 = item01
        ...
        if (move.isok) {
          ...
          this.Selection.map((item01, index01) => {
            if (item01.departmentid === move.departmentid) {
